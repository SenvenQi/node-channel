...
export class SessionManager implements ISessionManager {
    ...
    add<T>(sessionOptions: ClientArgs<T>): string {
        ...
        return this.addSession(sessionOption.session, sessionOption.channel, [
            sessionOptions.channelOptions.options,
            sessionOptions.channelOptions.filter,
        ]);
    }
    private addSession(
        ctor: SessionClientConstructor,
    ...
    private getOrThrow(id: string): Session {
        ...
        if (!session) throw new Error(`No session found for id: ${id}`);
        return session;
    }
    ...
    connect(sessionId: string): Promise<boolean> {
        ...
        else throw new Error("SessionServer not support connect");
    }
}
